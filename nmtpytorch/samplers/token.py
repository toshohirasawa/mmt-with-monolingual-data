...
class TokenBatchSampler(Sampler):
    ...
    def __init__(self, batch_size, sort_lens,
        ...
        # sort_lens: list of sentence length
        ...
        # buckets: key=length, value=[list of sent_id]
        ...
        for len_ in self.buckets:
            ...
            bucket_size = self.batch_size // len_
            self.bucket_sizes[len_] = bucket_size

            bucket_bs = np_bucket.size / bucket_size
    ...
    def __iter__(self):
        ...
        for len_ in shuf_idxs:
            # Get offset pointer for this bucket: 0 initially
            offset = bucket_offsets[len_]
            bucket_size = self.bucket_sizes[len_]

            # Convert them to permuted view
            idxs = bucket_views[len_][offset: offset + bucket_size]

            # Increment offset
            bucket_offsets[len_] += len(idxs)

            # Get actual sample indices
            sidxs = self.buckets[len_][idxs]
